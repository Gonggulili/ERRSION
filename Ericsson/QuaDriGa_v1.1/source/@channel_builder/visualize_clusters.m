function visualize_clusters( h_cb, i_mobile , i_cluster )
%VISUALIZE_CLUSTERS Plots the scattering clusters for a mobile terminal
%
%   This method plots all scattering clusters for a given mobile terminal.
%   If "i_cluster" is not given, then only the mail paths are shown for all
%   MPCs. If "i_cluster" is given, then also the subpaths are shown for the
%   selected cluster.
%
%   The plot is in 3D coordinates. You can rotate the image using the
%   rotate tool.
%
...
if ~exist( 'i_cluster', 'var' )
    i_cluster = 0;
end

tx_pos = h_cb.par.tx_position;
rx_pos = h_cb.par.rx_track( i_mobile ).initial_position;

[ ~, ~ , lbs_pos , fbs_pos ] = h_cb.calc_scatter_positions( i_mobile );
nNLOS = size(lbs_pos,2)-1;

lbs_center = mean( lbs_pos(:,2:end,:),3 );
fbs_center = mean( fbs_pos(:,2:end,:),3 );

[ ahat_x, ahat_y, ahat_z ] = sph2cart(h_cb.AoA(i_mobile,:), h_cb.EoA(i_mobile,:), 1);
ahat_x = ahat_x + rx_pos(1);
ahat_y = ahat_y + rx_pos(2);
ahat_z = ahat_z + rx_pos(3);

[ bhat_x, bhat_y, bhat_z ] = sph2cart(h_cb.AoD(i_mobile,:), h_cb.EoD(i_mobile,:), 1);
...
if i_cluster == 0
    ...
    for i_path = 1:numel( i_cluster )
        i_cl = i_cluster( i_path )+1;
        for i_sub = 1:20
            
            % Plot First LBS and FBS position
            plot3( fbs_pos(1,i_cl,i_sub),fbs_pos(2,i_cl,i_sub),fbs_pos(3,i_cl,i_sub) ,...
                '+r','Linewidth',2,'Markersize',7 );
            plot3( lbs_pos(1,i_cl,i_sub),lbs_pos(2,i_cl,i_sub),lbs_pos(3,i_cl,i_sub) ,...
                'ob','Linewidth',2,'Markersize',7 );
            
            % Plot path from Tx to FBS
            plot3( [tx_pos(1),fbs_pos(1,i_cl,i_sub)],...
                [tx_pos(2),fbs_pos(2,i_cl,i_sub)],...
                [tx_pos(3),fbs_pos(3,i_cl,i_sub)] ,':r' );
            
            % Plot path from Tx to FBS
            plot3( [rx_pos(1),lbs_pos(1,i_cl,i_sub)],...
                [rx_pos(2),lbs_pos(2,i_cl,i_sub)],...
                [rx_pos(3),lbs_pos(3,i_cl,i_sub)] ,':b' );
            
            % Plot path from Tx to FBS
            plot3( [fbs_pos(1,i_cl,i_sub),lbs_pos(1,i_cl,i_sub)],...
                [fbs_pos(2,i_cl,i_sub),lbs_pos(2,i_cl,i_sub)],...
                [fbs_pos(3,i_cl,i_sub),lbs_pos(3,i_cl,i_sub)] ,':m' );
        end
    end
end
